...
from summary_module import CommentSummarizer
...
def process_video(video_id, api_key):
    try:
        ...
        print(f"  Conflicted results saved to 'conflicted_sentiments/{conflict_output_file}'")
                # 🔽 ORTAK FİKİR ÖZETİ EKLENTİSİ 🔽
        summarizer = CommentSummarizer(num_clusters=5)
        summaries = summarizer.summarize(english_comments)

        summary_output_file = f"summaries_{sanitize_filename(title)}.json"
        save_json_to_folder(summaries, summary_output_file, "summaries")
        print(f"  Comment summaries saved to 'summaries/{summary_output_file}'")
